...
- The ingester still parses model 2.20. The model here is at 2.25. Porting it (CR type, field trim, risk Kind, I20 forward transitions, change set apply stage, and the supports rules I21 to I23 with the link words `mitigated by` and `needs`) is Adam's call and happens in the other repository.
