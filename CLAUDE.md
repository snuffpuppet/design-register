...
## Change proposals (2.35)

Internal Change proposals use CP IDs and `change-proposals/`; vendor CR references remain external. Migrate legacy internal CRs with the preview/apply CLI, preserving numeric suffixes, statuses, original history and vendor references while updating structured links and aliases. Source must name the solution design; `based on DEC-…` optionally references the chosen approach. Requirements have optional Options. `proposal_upgrade.py` previews by default and applies with an expected token and named operator inside a recovery-journal transaction. Stop all writers before migration; never perform a global CR text replacement in engagement data. See `docs/change-proposal-upgrade.md` for work-laptop rollout. Do not declare the external ingester ported or silently rewrite old records.
