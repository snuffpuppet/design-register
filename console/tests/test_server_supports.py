...
def write_item(eng, kind, id, title, status, **f):
    d = {"REQ": "requirements", "DEC": "decisions", "LIM": "limitations", "CR": "change-requests", "OI": "open-items", "RSK": "risks"}[kind]
...
class Stub:
    ...
    evidence = S.H.evidence; support_link = S.H.support_link
...
class LiveLink(unittest.TestCase):
    ...
    def test_offer_carries_matches_from_the_registers(self):
        self.assertEqual([m["id"] for m in self.s6()["matches"]], ["CR-0001"])

    def test_link_writes_an_edit_block_each_way_and_the_offer_goes(self):
        s = self.s6()
        r = Stub().support_link({"id": "LIM-0001", "key": s["key"], "target": "CR-0001", "madeBy": "Adam"})
        self.assertTrue(r["ok"])
        cs = S.load_change_sets()[0]
        self.assertEqual(len(cs["blocks"]), 2)
        self.assertEqual(cs["blocks"][0]["fields"]["Target"], "LIM-0001"); self.assertIn("dispositioned by CR-0001", cs["blocks"][0]["links"])
        self.assertEqual(cs["blocks"][1]["fields"]["Target"], "CR-0001"); self.assertIn("triggered by LIM-0001", cs["blocks"][1]["links"])
    ...
    def test_link_refuses_the_wrong_kind_and_an_unknown_target(self):
        ...
        with self.assertRaises(ValueError): Stub().support_link({"id": "LIM-0001", "key": s["key"], "target": "CR-0099", "madeBy": "Adam"})
