(function () {
  ...
  function Bulk() {
    ...
    const fields = common(kinds.map(k => ["title", ...(S.ui.mode === "rationalise" ? ["status", "raised-on", "closed-on", "description"] : []), ...S.model.short[k], ...S.model.long[k]]));
    ...
    const opts = k => (k === "status" ? common(kinds.map(kind => S.model.states[kind])) : null) || S.model.choices[k] || (k === "scope" ? S.model.scopes : k === "phase" ? S.model.phases : k === "owner" ? S.model.owners : null);
    ...
    return html`<div class="bar bulk">
      ...
      ${mode === "set" ? html`<select class="inp" value=${field} onChange=${e => { setField(e.target.value); setValue(""); }}><option value="">field…</option>${fields.map(f => html`<option value=${f}>${S.model.labels[f] || f}</option>`)}</select>
        ${field && (field === "status" || opts(field)?.length) ? html`<select class="inp" value=${value} onChange=${e => setValue(e.target.value)}><option value="">–</option>${opts(field).map(o => html`<option value=${o}>${o}</option>`)}</select>` : html`<input class="inp" value=${value} onInput=${e => setValue(e.target.value)} placeholder="value" />`}
        <button class="btn pri" disabled=${!field || (field === "status" && !value)} onClick=${guarded(() => bulk({ op: "set", fields: { [S.model.labels[field] || field]: value }, gist: (S.model.labels[field] || field) + " set in bulk" }))}>Apply to ${items.length}</button>` : null}
