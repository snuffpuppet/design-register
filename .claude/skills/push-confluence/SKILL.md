...
description: Use when asked to push, write back, publish or sync an engagement's registers to the Confluence Design Register pages for a named engagement. Builds the pages first and sends nothing until the write gate is passed.
usage: "/push-confluence <engagement>: build the registers as pages, read the manifest, send under the write gate"
---

# Push the registers back to Confluence

Send the engagement's registers, as they stand, back to the Confluence pages they were pulled from. Two halves: build, which writes files and sends nothing, then send, which is gated. Item files are never written by this skill.
...
2. Run `make push-pages ENG=engagements/<engagement>`. It runs `console/push-pages.py` in the console image and refuses on its own if the engagement is wrong, the pages came from another parent, no pull is logged. If Docker is not running, say so and stop; do not run host python.
...
- `mode` `replace-tables`: update the page `id` with the `body` from its file, keeping the title, with version = current version + 1 and a version message `Register as at <date> by <user>`.
...
Never delete a page. After each write, record the returned version and page id in the manifest entry as `sent_version` and `sent_id`, and write `sent_version` into the pulled page file's `page-version` line under `engagements/<engagement>/baseline/`, so the next build's moved-since check compares against the version the console itself sent rather than the original pull. If a write fails, stop, leave the manifest as it is, and report which pages were sent and which were not; the pages already sent stay sent, since Confluence keeps their history.
...
- Push twice without a fresh build.
