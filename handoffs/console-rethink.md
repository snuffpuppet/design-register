...
1. **Merge `console-rethink` to `main`.** Nothing in the branch is provisional; every task closed with a clean review in the ledger. `console/static/old/` and its route stay after the merge, on purpose.
