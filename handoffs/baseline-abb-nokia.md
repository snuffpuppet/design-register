...
- The supports engine is built and verified end to end on the sample (model 2.25, `console/integrity.py`). Baseline mode has a Missing supports tab after Row by row: every accepted row whose state implies another record (the decision behind an accepted limitation, the change request behind one marked Change requested, the requirement a limitation constrains, the open item behind a draft or a mitigation) is offered prefilled. Verdicts are Accept, Edit then accept, Dismiss with a reason, and for limitations Reconstruct or Reassess. The freeze refuses while a failure-level offer is undecided and lists implied items in `baseline/frozen.md`. Live mode offers the same on each item and in the move dialog, and writes ticked offers as change set blocks before the move. `make test` runs 65 unit tests in the python image. Nothing fills Approved by and no owner is ever guessed. Known gap: an offer can only create a new record; it cannot link an existing accepted candidate of that type. On the real pages the decisions register already holds a decision for many accepted limitations with no link between them, so the tab would offer duplicates.
...
5. Build the Link existing verdict before the real baseline run: each offer lists accepted candidates of the offered type whose title shares tokens with the trigger (the duplicates tab's Jaccard match) or that a source column names, and one click writes the link on the trigger and the reverse where the model names one, creating nothing. The same picker on the Live Supports panel through an edit block. Candidate finding is a lookup, no model call; the agent reads page prose only when asked to confirm an ambiguous match. Done when a limitation the source calls Accepted can be linked to an existing decision from the tab and the freeze accepts it.
