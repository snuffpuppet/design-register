...
6. **One table, one generated form.** The register views, triage queue, rationalise tabs and report pages were replaced by one filterable table with a side panel and a bulk bar, so that a single field, a whole item and a selection are edited the same way, and every state change asks only for what `model.py` says the move needs. The Baseline screens were out of that rethink's scope and stay in `console/static/old/` until ported.
