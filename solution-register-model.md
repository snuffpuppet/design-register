...
Version 2.35, 21 September 2026. Owner: Adam Moyes.

Version 2.35 uses CP identifiers and change-proposals folders for internal Change proposals, distinguishing them from vendor-managed CRs. The journalled migration maps existing internal CR IDs to CP, rewrites structured references and retains aliases and original evidence/history. The change-set contract is now 2.35; the external ingester must be ported before writing migrated engagements. This supersedes the display-only compatibility approach in 2.34.

Version 2.34 names the internal CR record Change proposal. It tracks work identified through iterative requirements and design development, with scope, phase, estimate, approval and delivery visibility; a vendor change request is an externally managed reference. Existing CR IDs, change-requests folders and statuses remain compatible. Requirements gain optional lightweight Options and proposals may link to a Decision with based on. No existing item is rewritten on upgrade.

Version 2.33 adds previewed type changes and lead-first merges to Rationalise, and restorable deletion. Version 2.32 adds immediate single and bulk field corrections in Rationalise and scope-filtered Desktop work. The structured review process remains optional. Version 2.31 separates historical rationalisation from ongoing workflow. Section 12 defines review corrections, independent review progress, meeting sessions and stable published identifiers. Imported rows become working registers immediately; there is no separate freeze stage. Item fields and the section 11 change-set wire format remain unchanged.
...
An open item closes only by creating or changing a record: a decision gets accepted, a limitation gets dispositioned, a change proposal gets raised, a requirement gets clarified, a risk gets retired. In a meeting we review one filtered list of open items. The registers behind them stay stable.
...
| Ask | A stakeholder wants X changed. | OI, then CP or REQ |
...
| Field | Rule |
|---|---|
| ID | Type prefix plus a four-digit zero-padded number: REQ-0014, DEC-0003, LIM-0021, RSK-0007, OI-0045, CP-0002. Never reused. Transcript ids are three digits (T001), episode ids are the transcript id plus E and a number (T001-E02), change set ids are CS plus four digits (CS-0003). |
| Title | One line, specific. |
| Status | One of the values for the type (4.2). |
| Owner | The one person field. On a requirement it is who stated the need and can say it is met. On an open item it is who does the work. On a decision, limitation, risk or change proposal it is who raised it, and the work that moves the item lives on an open item whose Owner is the worker. A named person on our side, "Vendor: <name>", "Joint", or a Forum row from the stakeholder register. Required while the item is not in a terminal state. |
| Implemented by | Vendor, Internal or Both. Whose build the item lands in. Required on requirements, decisions and change proposals. On a limitation it follows from the chosen option, so it is optional until the limitation is Accepted or Change requested. Not used on risks or open items. |
...
| Vendor ref | The number or link of an externally managed vendor change request, once created. Used on change proposals only. A vendor document reference on any other type goes in Source. |
...
| Change proposal | CP | Proposed, For approval, Approved, Submitted, Deferred, Delivered*, Withdrawn*, Rejected* | Reason (the business justification, in one or two sentences: what the change buys and for whom, written for the approver even where the requirement says it at length), Chosen option (one line: what will be built, chosen from the options on the triggering limitation or requirement), Estimate (the cost and time being approved, as `<Indicative or Confirmed>; <cost>; <duration/effort>; <source and date of the estimate>`), Approved by (the person or forum that approved the estimate), Phase (the named phase the change lands in) |

Dropped from earlier versions and why: the requirement's Deferred state said what Phase already says. The open item's In progress state carried nothing that Next action does not. The change proposal's Options state, Options list, Consulted and CP page repeated the work already done on the limitation; the CP decides how, and the how is one line on the row with the design on a page named in Source. For approval stays, because the change proposal is what the business sees when it approves cost, and the register must show what is waiting on them.
...
| Change proposal | Outstanding work identified as requirements and solution design develop iteratively, for this phase or a named later phase. It may be essential to delivery and does not require a previously complete or approved baseline. One internally managed row for the whole life of the proposal, linked to the solution design through Source. A vendor change request is managed by the vendor; its number or link goes in Vendor ref once created, where required for vendor delivery. Internal, Vendor and Both work are supported. It is the artefact the business approves: Reason is the justification in the approver's terms, Estimate is the number they approve, and Approved by is who approved it. Lightweight Options stay on the triggering requirement or limitation; a Decision records their selection where needed. The proposal carries the resulting work, approval and delivery position without duplicating the option analysis. |
...
- **Change proposal.** Links carries "triggered by" the limitation or requirement whose chosen option asked for the change. Proposed means ours and being shaped: Reason and Chosen option are written, the design for the chosen option is put on a page named in Source, and the estimate is obtained. For approval means the CP is with the business: Reason, Chosen option and Estimate must be filled before it enters, and it stays there until the approver acts. Approved needs Approved by, Closed on and Phase. Submitted is the retained status name for handover to whoever will implement it; it does not mean every proposal has a vendor request; with Implemented by Vendor, Submitted needs a Vendor ref. Delivered is set when the change is built and the requirement it delivers moves. Deferred means the change will be made in a named later phase and is waiting for it: it needs Phase, Approved by and Closed on, carries no open item, and is reviewed at phase planning, when it moves to Proposed and the work resumes on the same row. Withdrawn means we chose not to pursue the change after all, usually because the estimate made a workaround the better option; the triggering limitation returns to Under assessment, or the triggering requirement's open item reopens. Rejected means whoever approves or implements it said no; the same return applies. Approved, Deferred, Withdrawn and Rejected each need Approved by and Closed on. A CP in Proposed, For approval or Submitted must have an open item in Links carrying the worker and next action; while For approval, the open item's next action is the approval meeting or the chase. A change with Implemented by Both stays one row unless the vendor part and the internal part are approved separately, in which case it is two rows linked "part of".

A requirement may carry optional lightweight Options in an Options section, just as a limitation carries its alternatives. Use the solution design for detailed analysis. Each change proposal must reference the solution design and relevant section in Source; an existing Source citation must be reviewed by a person to confirm it is a design reference. Blank or unrelated Source text must not be treated as design evidence.

For SLT, group outstanding proposals by Scope and Phase. Estimate records cost, duration/effort, source and date, and explicitly states Indicative or Confirmed. Blank means Not sized, never zero. Do not sum free-text estimates containing different currencies, periods or effort units. Proposed can remain unsized; approval still requires an estimate. Deferred work is shown separately by phase.
...
Do not write a decision to restate a requirement, to put a requirement in or out of a phase, for the vendor's routine implementation detail, or to approve a change proposal. The CP's Approved by is that decision.
...
- The platform holds one notification channel per customer and a Must requirement needs email and SMS on day one. Limitation constraining that requirement, assessed through an open item. A Decision records acceptance of the shortfall or an option choice where needed. A change proposal tracks adding the second channel, with a vendor change request reference when created.
...
| LIM | dispositioned by | DEC or CP (a later disposition keeps the earlier one as "previously dispositioned by") |
| LIM, DEC, CP, REQ | assessed by, proposed by, worked by | OI (the open item carrying the work while the record is not terminal) |
| RSK | realised as | OI |
| OI | resolves into | any, or "none: <reason>" |
| CP | triggered by | LIM or REQ |
| CP | delivers | REQ |
| CP | based on | DEC (optional reference to the option-selection decision) |
| CP | part of | CP (when a Both change is split into a vendor row and an internal row) |
...
Internal Change proposals use CP IDs and the `change-proposals/` directory. Vendor-managed change requests retain their external CR references in Vendor ref. Existing internal CR records must be migrated using the journalled preview/apply procedure in `docs/change-proposal-upgrade.md`; numeric suffixes, history and source evidence are preserved, structured links are rewritten, and aliases resolve old internal IDs. Existing status values, including LIM Change requested and CP Submitted, remain unchanged; Change requested means dispositioned through a proposal, not proof of vendor submission.

One file per item, named by its id, in a folder per type: `requirements/REQ-0004.md`, `decisions/`, `limitations/`, `risks/`, `open-items/`, `change-proposals/`. The file opens with a YAML frontmatter block holding the header fields in 4.1 and the short type-specific fields, in kebab-case (`raised-on`, `closed-on`, `implemented-by`, `vendor-ref`, `links` as a list). Long fields sit in the body under fixed headings: Source (one citation or reference per line), Rationale, Impact, Options, Reason, Trigger, Mitigation, Next action, Notes, and optionally History, one line per write made by a tool in the form `date | person | move | gist | evidence`. A change to one item is a change to one file, and the item's history is that section together with the file's history in version control.
...
| CP | id, title, status, scope, owner, chosen-option, estimate, approved-by, phase, implemented-by, vendor-ref, links, raised-on, closed-on, updated |
...
4. Change proposals in For approval first, then Proposed and Submitted.
5. Decisions in Proposed older than 14 days.
6. Requirements in Draft older than 14 days, measured from Raised on.

Requirements whose Phase is later than the current phase, and change proposals in Deferred, are excluded from this view. They appear on a later-phase view: deferred CPs grouped by Phase, each with the limitation or requirement that triggered it, then requirements by Phase. That view is reviewed at phase planning, when each deferred CP for the starting phase moves to Proposed and gets an open item.
...
| I2 Valid status and required fields | Every status is an exact 4.2 value for its type. Every item has Raised on. Every requirement has MoSCoW. Every limitation has Impact once past Identified, and at least two Options, a Chosen option and Implemented by once Accepted or Change requested. Every risk has Kind, and Trigger and Mitigation once Mitigating. Every change proposal has Reason, Chosen option and Estimate once past Proposed; Approved by and Phase once Approved, Submitted, Delivered or Deferred; and Vendor ref once Submitted with Implemented by Vendor. Every item in an approved or terminal state has Closed on. |
| I3 Owner present | Every non-terminal item has an Owner that resolves under I19. A requirement in Draft, a decision in Proposed, a limitation in Under assessment and a change proposal in Proposed, For approval or Submitted must each have an open item in Links whose Owner is set. |
...
| I10 Change proposal approval | Every CP in Approved, Submitted, Delivered, Deferred, Withdrawn or Rejected has Approved by and Closed on. Every CP has a "triggered by" link to a LIM or REQ, and a CP triggered by a LIM in Change requested is that LIM's disposition. |
| I11 Decision approval | Every DEC in Accepted or Rejected has Approved by, Closed on and at least one Consulted entry. |
| I12 Implemented by | Every REQ, DEC and CP, and every LIM in Accepted or Change requested, has Implemented by set to Vendor, Internal or Both. |
...
| I17 Deferred change proposal | Every CP in Deferred has a Phase naming a later phase and no open item in Links. |
...
| I20 Forward transitions | A change to an existing item's Status follows an arrow in 4.4. The allowed return moves are LIM from Change requested to Under assessment, CP from Deferred to Proposed, and OI between Open and Blocked. Anything else is a failure and needs a new item, not an edit. |
...
| I23 Delivered change | Every CP in Delivered has a "delivers REQ-nnnn" link. |
...
Import creates working records and a review immediately. Source provenance and the source-to-item map are retained. There is no separate freeze stage. Published IDs stay stable; merges and retyping preserve aliases to surviving records. Splitting preserves the original reference as an index of the new records. Deleted records and removed incoming links are retained in a private rubbish bin. Restoration recovers the original ID and history and adds back removed incoming links without overwriting later field edits. Deleted identifiers are reserved rather than reused. Immediate merges keep the chosen lead's populated values, fill empty fields and retain conflicting values and previous history in Notes. Type changes preserve prior fields and history, rewrite incoming links and retain old-ID aliases. Both operations show before/after previews and reject acceptance if the source records changed. Renumbering is reserved for explicitly unpublished registers.
