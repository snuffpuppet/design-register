...
Version 2.34, 21 September 2026. Owner: Adam Moyes.
...
Do not write a decision to restate a requirement, to put a requirement in or out of a phase, for the vendor's routine implementation detail, or to approve a change proposal. The CR's Approved by is that decision.
...
| CR | delivers | REQ |
| CR | based on | DEC (optional reference to the option-selection decision) |
...
The display name Change proposal retains the CR prefix and `change-requests/` directory for compatibility. Existing status values, including LIM Change requested and CR Submitted, also remain unchanged; Change requested means dispositioned through a proposal, not proof of vendor submission. No file, link or history migration is needed.
...
| I10 Change proposal approval | Every CR in Approved, Submitted, Delivered, Deferred, Withdrawn or Rejected has Approved by and Closed on. Every CR has a "triggered by" link to a LIM or REQ, and a CR triggered by a LIM in Change requested is that LIM's disposition. |
| I11 Decision approval | Every DEC in Accepted or Rejected has Approved by, Closed on and at least one Consulted entry. |
| I12 Implemented by | Every REQ, DEC and CR, and every LIM in Accepted or Change requested, has Implemented by set to Vendor, Internal or Both. |
...
| I17 Deferred change proposal | Every CR in Deferred has a Phase naming a later phase and no open item in Links. |
