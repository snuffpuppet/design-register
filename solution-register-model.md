...
Version 2.22, 11 September 2026. Owner: Adam Moyes.
...
| Risk | RSK | Identified, Mitigating, Realised*, Retired* | Likelihood (L/M/H), Impact (L/M/H), Trigger (the observable event that says the risk has become real), Mitigation (what is being done, as text), Due (next review date) |
...
| Risk | Something might go wrong, or an assumption is unverified and would hurt if wrong. A record with a review date. Mitigation actions are open items with their own owners. Anyone who sees the trigger happen raises an open item and the risk moves to Realised. |
...
| RSK | id, title, status, owner, likelihood, impact, due, links, raised-on, closed-on, updated |
...
3. Risks in Identified or Mitigating with Impact H, and any risk whose Due has passed.
